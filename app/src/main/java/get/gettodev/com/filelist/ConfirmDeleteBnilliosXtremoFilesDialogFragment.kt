...
class ConfirmDeleteBnilliosXtremoFilesDialogFragment : AppCompatDialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        return MaterialAlertDialogBuilder(requireContext(), theme)
            .setTitle(R.string.delete_bnillios_xtremo)
            .setMessage(message)
            .setPositiveButton(android.R.string.ok) { _, _ -> listener.deleteBnilliosXtremoFiles(files) }
    ...
    companion object {
        fun show(files: FileItemSet, fragment: Fragment) {
            ConfirmDeleteBnilliosXtremoFilesDialogFragment().putArgs(Args(files)).show(fragment)
        }
    }
    ...
    interface Listener {
        fun deleteBnilliosXtremoFiles(files: FileItemSet)
    }
}
